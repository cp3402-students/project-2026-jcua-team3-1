...
function tennis_club_register_recommended_plugins() {
	$plugins = array(
		array(
			'name'             => __( 'WooCommerce', 'tennis-club' ),
			'slug'             => 'woocommerce',
			'required'         => false,
			'force_activation' => false,
		),
		array(
			'name'             => __( 'Creta Testimonial Showcase', 'tennis-club' ),
	...
	$config = array();
	tennis_club_tgmpa( $plugins, $config );
}
add_action( 'tgmpa_register', 'tennis_club_register_recommended_plugins' );
...
function install_and_activate_creta_testimonial_plugin() {
    ...
    $tennis_club_upgrader = new Plugin_Upgrader(new Automatic_Upgrader_Skin());
    ...
    $tennis_club_installed_plugins = get_plugins();

    foreach ($plugins as $plugin) {
        // Install if not present
        if (!isset($tennis_club_installed_plugins[$plugin['file']])) {
            $tennis_club_install_result = $tennis_club_upgrader->install($plugin['url']);
            if (is_wp_error($tennis_club_install_result)) {
        ...
        if (!is_plugin_active($plugin['file'])) {
            $tennis_club_activate_result = activate_plugin($plugin['file']);
            if (is_wp_error($tennis_club_activate_result)) {
                wp_send_json_error([
                    'message' => "Failed to activate {$plugin['slug']}",
                    'error'   => $tennis_club_activate_result->get_error_message(),
                ]);
            }
        }
    }
